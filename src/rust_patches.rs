...
impl RustPatch {
    pub fn new_rand<T>(rng: &mut T, window_rect: Rect, max_size: f32, colour: Hsla) -> Self
    where
        T: RngCore,
    {
        let position = pt2(
        ...
        let ellipse: Vec<Vec2> = geom::Ellipse::new(geom::Rect::from_wh(vec2(1.0, 1.0)), window_rect.right())
            .circumference()
            .into_iter()
            .map(|[x, y]| vec2(x, y))
            .collect();
